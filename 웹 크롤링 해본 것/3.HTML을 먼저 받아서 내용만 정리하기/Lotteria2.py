...
with open(os.getcwd() + "/working_htmls/Lotteria.html", mode='r', encoding='utf8') as f:
    html = f.read()
nameList = []
phoneList = []
addressList = []
...
address1 = soup.find_all("p", class_="newAddress", attrs={'data-id':'newaddr'})
address2 = soup.find_all("span", class_=False,attrs={'data-id':'address'})
address = []
...
for i in address1:
    if i.string is not None:
        ...
        addressList.append(address2[temp].string)
        temp += 1
...
for line, phone in enumerate(phoneList):
    ws.cell(row=line + 1, column=8).value = phone
for line, address in enumerate(addressList):
    ws.cell(row=line + 1, column=9).value = address
...
print(address1)
print(address2)
wb.save("C:/Users/gjdigj145/Lotteria.xlsx")
wb.close()

...
with open(os.getcwd() + "/working_htmls/BurgerKing.html", mode='r', encoding='utf8') as f:
    html = f.read()
...
name = soup.find_all("a", class_="gotostore")
phone = soup.find_all("span", class_='call')
address = soup.find_all("span", class_="address")

for i, j in enumerate(name):
    nameList.append(j.string)
for i in phone:
...
for i in range(len(nameList)):
    print("No.{} 매장명 :{}, 연락처 :{}, 주소 :{}".format(i+1, nameList[i], phoneList[i], addressList[i]))
...
for line, name in enumerate(nameList):
    ws.cell(row=line + 1, column=1).value = line+1
    ws.cell(row=line + 1, column=2).value = name
for line, phone in enumerate(phoneList):
    ws.cell(row=line + 1, column=7).value = phone
...
wb.save("C:/Users/gjdigj145/BurgerKing.xlsx")
wb.close()
